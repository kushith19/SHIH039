...
describe('sequential multi-incident orchestration', () => {
  ...
  it('does not start B until A is recovered; each gets its own LLM request', async () => {
    const llmCalls = []
    setLlmCommanderTestCaller(async (payload) => {
      llmCalls.push(payload.incident.incidentId)
      return {
    ...
    try {
      ...
      // Still awaiting approval — must not plan B yet
      ...
      assert.equal(afterB.advanced, true)
      assert.equal(room.responseOrchestration.plan.primaryIncidentId, 'inc-c')
      assert.deepEqual(llmCalls, ['inc-a', 'inc-b', 'inc-c'])

      const afterC = await approveAndRecover(room, 'inc-c')
      assert.equal(afterC.completed, true)
      assert.equal(afterC.advanced, false)
      assert.equal(
        room.responseOrchestration.orchestrationCycleStatus,
        ORCHESTRATION_CYCLE_STATUS.COMPLETED
      )
  ...
  it('human approval pauses the queue (no second LLM until recovered)', async () => {
    const llmCalls = []
    setLlmCommanderTestCaller(async (payload) => {
      llmCalls.push(payload.incident.incidentId)
      return {
    ...
    try {
      ...
      assert.equal(approved.ok, true)
      // Approval alone must not advance to B
      assert.equal(llmCalls.length, 1)
      assert.equal(room.responseOrchestration.plan.primaryIncidentId, 'inc-a')
      assert.ok(
  ...
  it('skips incidents that disappear before their turn', async () => {
    const llmCalls = []
    setLlmCommanderTestCaller(async (payload) => {
      llmCalls.push(payload.incident.incidentId)
      return {
    ...
    try {
      ...
      room.detection.anomalyNodeIds = ['a', 'c']
  ...
  it('does not rebuild queue when merely selecting focus mid-cycle', () => {
    const room = roomWithIncidents([
      ['inc-a', 'a'],
      ['inc-b', 'b'],
    ])
    beginOrchestrationCycleQueue(room, { focusIncidentId: 'inc-a' })
    const first = [...room.responseOrchestration.orchestrationQueue]
    beginOrchestrationCycleQueue(room, { focusIncidentId: 'inc-b' })
    assert.deepEqual(room.responseOrchestration.orchestrationQueue, first)
    assert.equal(room.responseOrchestration.currentIncidentId, 'inc-a')
  })
...
describe('sequential orchestration (deterministic planner)', () => {
  ...
  it('advances A → B only after recovery with deterministic plans', async () => {
    const room = roomWithIncidents([
      ['inc-a', 'a'],
      ['inc-b', 'b'],
    ])
