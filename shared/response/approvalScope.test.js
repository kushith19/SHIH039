...
describe('approvalScope STEP 9', () => {
  it('buildApprovalScope snapshots active incidents and executable actions', () => {
    ...
    assert.ok(scope.actionTypes.includes('capture-device-state'))
    assert.equal(scope.autoContinue, true)
    assert.ok(Array.isArray(scope.missionCapabilities))
    assert.ok(scope.missionCapabilities.includes('isolate-node'))
    assert.ok(scope.missionCapabilities.every((actionId) => scope.actionTypes.includes(actionId)))
